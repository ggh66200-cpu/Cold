...
# 🌐 1. تشغيل سيرفر الويب فوراً في أول ثانية لتجاوز فحص منافذ Render بنجاح
app = Flask('')
...
def run_web():
    ...
    print(f"⚡ Flask server is starting on port {port}...", flush=True)
    app.run(host='0.0.0.0', port=port)

# تشغيل السيرفر في خيط (Thread) منفصل فوراً وقبل أي استيراد قد يكون معلقاً
t = threading.Thread(target=run_web)
t.daemon = True
t.start()
print("✅ Flask background thread started successfully!", flush=True)

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 🔑 2. جلب المفاتيح الخاصة بالبوت بأمان تام من السيرفر
...
# الآن نقوم باستيراد بقية المكاتب المطلوبة للبوت
import json
import telebot
...
# استيراد ملفات الاشتراك والتحكم الخاصة بك بأمان
try:
    import sub
    HAS_SUB_SYSTEM = True
    print("✅ Subscription system (sub.py) imported successfully.", flush=True)
except ImportError:
    HAS_SUB_SYSTEM = False
    print("⚠️ sub.py not found, bypassing subscription system.", flush=True)

try:
    import admin
    HAS_ADMIN_SYSTEM = True
    print("✅ Admin system (admin.py) imported successfully.", flush=True)
except ImportError:
    HAS_ADMIN_SYSTEM = False
    print("⚠️ admin.py not found, bypassing admin system.", flush=True)
...
def send_welcome(message):
    ...
    if HAS_SUB_SYSTEM:
        try:
            ...
            print(f"Sub check bypass error: {e}", flush=True)
...
if __name__ == '__main__':
    print("🤖 البوت متصل الآن بنجاح ويعمل بأقصى سرعة...", flush=True)
